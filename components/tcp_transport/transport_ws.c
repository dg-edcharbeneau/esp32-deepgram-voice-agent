...
static int ws_connect(esp_transport_handle_t t, const char *host, int port, int timeout_ms)
{
    ...
    /* LOCAL PATCH -- the only change from ESP-IDF's copy of this file.
     *
     * Upstream always writes "Host: <host>:<port>". RFC 7230 5.4 permits that,
     * but omitting a default port is the near-universal convention (browsers,
     * curl, and every WS library do it) and some edges route strictly on it:
     * agent.deepgram.com answers 404 to "Host: agent.deepgram.com:443" and 401
    ...
    int len = snprintf(ws->buffer, WS_BUFFER_SIZE,
                       ...
                       "Sec-WebSocket-Key: %s\r\n",
                       ws->path,
                       host, port_suffix, user_agent_ptr,
                       client_key);

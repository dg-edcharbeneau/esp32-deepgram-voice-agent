...
"sel.innerHTML='';"
"l.forEach(function(n){opt(n.ssid,n.ssid+'  ('+n.rssi+' dBm)'+(n.open?'  open':''))});"
...
static esp_err_t scan_handler(httpd_req_t *req)
{
    ...
    size_t cap = 64 + s_scan_count * (WIFI_CREDS_SSID_LEN * 2 + 48);
    ...
    size_t n = 0;
    json[n++] = '[';
    for (size_t i = 0; i < s_scan_count && n + 1 < cap; i++) {
        char esc[WIFI_CREDS_SSID_LEN * 2];
        json_escape(s_scan[i].ssid, esc, sizeof(esc));
        int w = snprintf(json + n, cap - n, "%s{\"ssid\":\"%s\",\"rssi\":%d,\"open\":%s}",
                         i ? "," : "", esc, s_scan[i].rssi,
...
static esp_err_t save_handler(httpd_req_t *req)
{
    ...
    /* Logged without the password, which is the whole reason this is a POST. */
    ESP_LOGI(TAG, "provisioned \"%s\" (%s)", ssid, pass[0] ? "with key" : "open");
...
esp_err_t wifi_prov_start(void)
{
    ...
    wifi_config_t ap_cfg = {
        .ap = {
            .authmode = WIFI_AUTH_OPEN,   /* see the header for why */
            .max_connection = 4,
            .channel = 1,
        },
    };
